...
以下は、コンテンツディレクトリ構造の内の任意のフォルダに、名前を付ける際の一般的な規則です。
...
[2.1.1](#2.1.1) を再実施したうえで、絶対にスペースを使わない。 スペースによって、さまざまなエンジニアリングツールやバッチ処理が失敗する可能性があります。 理想的には、プロジェクトのルートには空白が含まれておらず、 `C:\Users\My Name\My Documents\Unreal Projects` の代わりに `D:\Project`のような場所に置かれているのが理想的です。
...
> `Developers` フォルダは、プロジェクトが依存しているアセット用ではないため、プロジェクト固有のものではありません。 これに関する詳細は [2.2](#2.2)  を参照してください。
...
すべてのアセットは目的を持っている必要があります。そうでなければ、プロジェクトに属していません。 アセットが実験的なテストであり、プロジェクトで使用すべきでない場合は、 [`Developers`](#2.3) フォルダーに置く必要があります。 
